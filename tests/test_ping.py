...
def test_ping_host_parses_unix_success(monkeypatch):
    ...
    monkeypatch.setattr(
        ping.socket,
        "getaddrinfo",
        lambda host, *_args, **_kwargs: [(None, None, None, None, ("93.184.216.34", 0))],
    )
    ...
    monkeypatch.setattr(ping.subprocess, "run", lambda *a, **k: make_proc(stdout=unix_out, returncode=0))

    res = ping.ping_host("example.com", count=5, timeout=1)
...
def test_ping_host_parses_windows_success(monkeypatch):
    ...
    # Skip real DNS
    monkeypatch.setattr(
        ping.socket, "getaddrinfo", lambda *a, **k: [(None, None, None, None, ("1.2.3.4", 0))]
    )
    ...
    monkeypatch.setattr(ping.subprocess, "run", lambda *a, **k: make_proc(stdout=win_out, returncode=0))

    res = ping.ping_host("example.com", count=4, timeout=1)
    assert res["alive"] is True
...
def test_ping_host_handles_nonzero_exit(monkeypatch):
    ...
    monkeypatch.setattr(ping.subprocess, "run", lambda *a, **k: make_proc(stdout="no reply", returncode=1))

    res = ping.ping_host("bad.example", count=1, timeout=1)
...
def test_ping_hosts_aggregates(monkeypatch, capsys):
    ...
    res = ping.ping_hosts(
        ["ok.example", "down.example", "ok2.example"], count=3, timeout=1, print_live=True
    )
